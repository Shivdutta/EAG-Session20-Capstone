...
# Enhanced Fund Recommendation Streaming Endpoint with better session handling
@app.post("/api/fund-recommendation")
async def fund_recommendation_stream(request: Request, fund_request: Dict[str, Any] = Body(...)):
    """Stream fund recommendation progress with enhanced HTML report analysis and session-aware file tracking"""
    ...
    async def event_generator():
        try:
            ...
            fund_session_id = session_id  # Use the same session ID or create new
                ...
                async for event_data in agent_stream_service.process_query_stream(populated_template):
                    ...
                    try:
                        ...
                        if clean_data:
                            ...
                            if (event_json.get("type") == "agent_response" and 
                                any(keyword in message_content for keyword in ["fund_recommendation", "fund_recommendations.html", "fund_analysis", "recommendation_report", ".html"])):
                                ...
                                fund_patterns = [
                                    r'media[\\\/]generated[\\\/]([^\\\/]+)[\\\/][^\\\/]*fund[^\\\/]*\.html',
                                    r'media[\\\/]generated[\\\/]([^\\\/]+)[\\\/][^\\\/]*recommendation[^\\\/]*\.html',
                                    r'media[\\\/]generated[\\\/]([^\\\/]+)[\\\/][^\\\/]*\.html',
                                    r'([^\\\/]+[\\\/])*media[\\\/]generated[\\\/]([^\\\/]+)[\\\/][^\\\/]*\.html',
                                    r'[\w\\/.-]+\.html'
                                ]
                                ...
                                for pattern in fund_patterns:
                                    ...
                                    if path_match:
                                        ...
                                        # Use provided session ID or extract from path
            ...
            if not generated_fund_file_path:
                ...
                found_fund_file = find_latest_fund_recommendation_file_by_session(fund_session_id)
                if found_fund_file:
