...
async def fund_recommendation_stream(request: Request, fund_request: Dict[str, Any] = Body(...)):
    """Stream fund recommendation with progress bar, spinner, logs, and file display"""
    ...
    async def event_generator():
        try:
            ...
            # 1. PROGRESS BAR: Send initial connection (0%)
            initial_event = {
                "type": "connection_established", 
                "data": {
            ...
            # 2. PROGRESS BAR: Template processing (5%)
            try:
            ...
            # 3. PROGRESS BAR + SPINNER + STREAM LOGS: Agent execution (5%-95%)
            try:
                async for event_data in agent_stream_service.process_query_stream(populated_template):
                    if await request.is_disconnected():
                        break
                    
                    # CRITICAL: Forward ALL events for progress bar, spinner, and logs
                    yield event_data
                    
                    # FILE DISPLAY: Capture file generation info
                    try:
                        clean_data = event_data.replace("data: ", "").strip()
                        if clean_data:
                            event_json = json.loads(clean_data)
                            
                            # Look for completion with file info
                            if event_json.get("type") == "stream_complete":
                                completion_data = event_json.get("data", {})
                                if "generated_file_path" in completion_data:
                                    generated_fund_file_path = completion_data["generated_file_path"]
                                    fund_session_id = completion_data.get("session_id")
                                    generated_fund_filename = "comprehensive_report.html"
            ...
            # 4. FILE DISPLAY: Find generated file if not captured
            if not generated_fund_file_path:
                print("Searching for fund recommendation files...")
                
                # Find most recent file that's NOT the SIP file
                found_file = find_latest_fund_recommendation_file_by_session()
                if found_file:
                    generated_fund_filename, generated_fund_file_path = found_file
                    fund_session_id = extract_session_id_from_path(generated_fund_file_path)
                    
                    # Only use if different from SIP session
                    if fund_session_id != sip_session_id:
                        # FILE DISPLAY: Emit file found event
                        file_event = {
                            "type": "file_generated",
                            "data": {
            ...
            # 5. PROGRESS BAR: Final completion (100%)
            completion_event = {
                "type": "stream_complete",
                "data": {
                    ...
                    "result": {
                        ...
                        "fund_filename": generated_fund_filename,  # DISPLAY FILE NAME
